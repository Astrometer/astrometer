...
contract Owner {
   ...
   // owners and roles
    ...
    modifier isOwner() {
        require(hasOwner(), string(abi.encodePacked("Caller is not owner ", msg.sender.toAsciiString())));
        _;
    }
    ...
    function hasRole(string memory role) public view returns(bool) {
        ...
        for(uint256 i; i<=owners[msg.sender].length; ++i) {
            string memory str = owners[msg.sender][i].Role;
            if (keccak256(abi.encodePacked(str)) == keccak256(abi.encodePacked(role))) {
            //if(owners[msg.sender][i].Role == role) {
                return owners[msg.sender][i].active;
            }
        }
    ...
    function addRole(string memory role, address _address) public returns(bool){
        require(hasRole('CAN_ADD_ROLE'), string(abi.encodePacked("Caller has no permission ", msg.sender.toAsciiString())));
    ...
    function deleteRole(string memory role, address _address) public returns(bool){
        require(hasRole('CAN_DELETE_ROLE'), string(abi.encodePacked("Caller has no permission ", msg.sender.toAsciiString())));
        for(uint256 i; i<=owners[_address].length; ++i) {
            string memory str = owners[_address][i].Role;
            if (keccak256(abi.encodePacked(str)) == keccak256(abi.encodePacked(role))) {
...
contract AstroToken is ERC20, Owner {
    ...
    constructor () ERC20("Astrometer token", "AMTR") {
